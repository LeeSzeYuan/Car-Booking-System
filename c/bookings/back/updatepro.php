    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if (empty($start_err)&&empty($end_err)&&empty($brand_err)&&empty($vehicle_err)){
            ...
            if (in_array($vehicle, $unavailable)){
                $sql = "UPDATE tb_booking SET v_id = ?, b_date = ?, r_date = ?, total_price = ?, b_status = 1 WHERE book_id = $id;";
            }else{
                $sql = "UPDATE tb_booking SET v_id = ?, b_date = ?, r_date = ?, total_price = ?, b_status = 2 WHERE book_id = $id;";
            }
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)){
                ...
                if (mysqli_stmt_execute($stmt)){
                    ...
                    if (in_array($vehicle, $unavailable)){
                        echo "<script>alert('Your Booking changes is saved.But need to be reviewed by our company first'); window.location.replace('read.php');</script>";
                    }else{
                        echo "<script>alert('Your Booking changes is saved. And it is Approved!'); window.location.replace('read.php');</script>";
                    }
