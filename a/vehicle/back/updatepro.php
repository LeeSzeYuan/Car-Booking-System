<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if (empty($id_err)&&empty($name_err)&&empty($brand_err)&&empty($rental_err)&&empty($year_err)&&empty($errMSG)){
            ...
            if ($imgExt== ""){
                $sql = "UPDATE tb_vehicle SET v_id = ?, v_type = ?, b_id = ?, v_price = ?, v_year = ?, v_detail = ? WHERE v_id = '".$v_id."';";
            }else{
                $sql = "UPDATE tb_vehicle SET v_id = ?, v_type = ?, b_id = ?, v_price = ?, v_year = ?, v_detail = ?, v_img_path = ? WHERE v_id = '".$v_id."';";
            }
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)){
                ...
                if ($imgExt== ""){
                    mysqli_stmt_bind_param($stmt, "ssisss", $id, $name, $brand, $rental, $year, $detail);
                }else{
